...
class AppointmentParticipantExt extends StatefulWidget {
  const AppointmentParticipantExt(
  ...
  @override
  _AppointmentParticipantExtState createState() => _AppointmentParticipantExtState();
}

class _AppointmentParticipantExtState extends State<AppointmentParticipantExt>
    with TickerProviderStateMixin {
  AnimationController? animationController;
  List<ParticipantDataExt> appointmentParticipantExt = [];
  ...
  Future<String> getParticipant() async {
    ...
    try {
      var response = await http.post(Uri.parse("https://appointment.geloraaksara.co.id/api/list_participant_exp"),
          body: {
      ...
      for (var i = 0; i < dataParticipant['data'].length; i++) {
        ...
        var instansi = dataParticipant['data'][i]['instansi'].toString();
        var meeting_id = dataParticipant['data'][i]['meeting_id'].toString();

        appointmentParticipantExt.add(ParticipantDataExt(id, nik, nama, bagian, instansi, meeting_id));
      }
    } catch (e) {
      print("Error");
    }
                            ...
                            if (snapshot.hasError)
                              return GridView.builder(
                                  ...
                                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                                      ...
                                      crossAxisSpacing: 0,
                                      mainAxisSpacing: 10),
                                  itemCount: appointmentParticipantExt.length,
                                  itemBuilder: (BuildContext context, int index) {
                                    final int count = appointmentParticipantExt.length;
                                    ...
                                    return itemParticipant(appointmentParticipantExt[index], context, animationController!, animation);
                                  });
                            else
                            if(appointmentParticipantExt.length==0)
                              return
                                FadeInUp(
                                      ...
                                      children: [
                                        ...
                                        Column(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              'Data peserta eksternal tidak tersedia',
                                              textAlign: TextAlign.left,
                                              style: TextStyle(
                                                fontFamily: AppTheme.fontName,
                                                fontWeight: FontWeight.w500,
...
Widget itemParticipant(ParticipantDataExt participantData, BuildContext context, AnimationController animationController, Animation<double> animation){
  return
    AnimatedBuilder(
